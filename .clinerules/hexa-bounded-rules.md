...
- When creating new bounded contexts, follow this structure:
  ...
  - src/{context-name}/infrastructure/primary/ (Angular components, controllers)
...
## File Organization
- Angular components go in infrastructure/primary/ only
- Follow naming: {ContextName}Component.ts, {ContextName}Component.html, {ContextName}Component.scss
- Keep separate .ts, .html, .scss files as per Angular conventions
...
## Testing Compliance
- Run npm run test to verify architecture compliance with Jest
- Run npm run e2e-headless to run Cypress tests
...
- Angular components: PascalCase with Component suffix (UserProfileComponent)
- Use data-selector attributes for E2E test selectors

## Anti-Patterns to Reject
- Anemic domain models (just getters/setters)
- Business logic in Angular components or controllers
